/**
 * Reads the adversary and environment stat blocks out of the two books in
 * `docs/rules` and writes `src/packs-src/stat-blocks.json`.
 *
 *     node tools/extract-stat-blocks.mjs
 *
 * `docs/` is reference material and is not committed — 579MB of somebody
 * else's book, none of which the build needs. So this is `fetch-cards.mjs`'s
 * arrangement pointed at a local source instead of a network one: the parse
 * happens here, on a machine that has the books, and what the build reads is
 * the **committed snapshot**. A clean checkout compiles the packs with no
 * `docs/` anywhere, which is the whole point — the parser lived in
 * `src/packs-src/` for a while and made these two packs the only ones in the
 * system that could not be built from the repository.
 *
 * It does not police what it wrote. `tools/check-stat-blocks.mjs` is the
 * ratchet on the committed artefact and runs in both `typecheck` and
 * `build:packs`; a second copy of "290 adversaries" in here would be two
 * numbers to keep in step and only one of them guarding the build. What this
 * refuses is a parse that produced nothing at all, which is what a missing or
 * renamed chapter file looks like from in here.
 */

import { readFileSync, writeFileSync } from "node:fs";
import { dirname, join } from "node:path";
import { fileURLToPath } from "node:url";
import { featureItem, typeGlyph } from "../src/packs-src/_helpers.mjs";

const ROOT = join(dirname(fileURLToPath(import.meta.url)), "..");
const SNAPSHOT = join(ROOT, "src", "packs-src", "stat-blocks.json");
...
const ENVIRONMENTS = Object.values(BOOKS).flatMap((book) =>
  blocks(book, book.environments).map(environment),
);

if (!ADVERSARIES.length || !ENVIRONMENTS.length) {
  throw new Error(
    `Parsed ${ADVERSARIES.length} adversaries and ${ENVIRONMENTS.length} environments. ` +
      `Nothing was written — check that docs/rules still holds the chapters BOOKS names.`,
  );
}

/* Indent 1 and a trailing newline, the same as `official-cards.json`. The
   file is generated and nobody reads it by hand, but a snapshot that lands
   as one line makes every regeneration a single unreviewable diff. */
writeFileSync(SNAPSHOT, `${JSON.stringify({ adversaries: ADVERSARIES, environments: ENVIRONMENTS }, null, 1)}\n`);

const features = [...ADVERSARIES, ...ENVIRONMENTS].reduce((sum, entry) => sum + entry.items.length, 0);
console.log(
  `Wrote src/packs-src/stat-blocks.json — ${ADVERSARIES.length} adversaries, ` +
    `${ENVIRONMENTS.length} environments, ${features} embedded features.`,
);
